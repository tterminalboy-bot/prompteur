...
let running=false,offset=0,last=0,mirror=s.mirror,invert=s.invert,peer,hostCode,connections=new Set();
...
function render(){const p=qs('#prompt');p.textContent=text.value;p.style.fontSize=size.value+'vw';p.style.left=margin.value+'%';p.style.right=margin.value+'%';p.style.color=color.value;p.style.filter=invert?'invert(1)':'none';p.style.transform='translateY('+(-offset)+'px)'+(mirror?' scaleX(-1)':'')}
...
function play(v){running=v;if(v){last=0;requestAnimationFrame(loop)}qs('#play').textContent=v?'Ⅱ':'▶';broadcast({type:'state',state:state()})}
...
qs('#mirror').onclick=()=>{mirror=!mirror;save()};qs('#invert').onclick=()=>{invert=!invert;save()};qs('#fullscreen').onclick=()=>document.documentElement.requestFullscreen?.();qs('#exit').onclick=()=>{play(false);qs('#stage').hidden=true;qs('#setup').style.display='block'};
qs('#launch').onclick=()=>{if(!text.value.trim())return toast('Ajoutez un texte.');const n=+count.value;if(!n)return openStage();qs('#count').hidden=false;let x=n;qs('#countNum').textContent=x;const t=setInterval(()=>{x--;qs('#countNum').textContent=x;if(x<=0){clearInterval(t);qs('#count').hidden=true;openStage()}},1000)};
function openStage(){qs('#setup').style.display='none';qs('#stage').hidden=false;render()}
...
function host(){hostCode=makeCode();peer=new Peer('pt-'+hostCode);peer.on('open',()=>{qs('#code').textContent=hostCode;qs('#qr').innerHTML='';new QRCode(qs('#qr'),{text:location.href.replace(/prompter\.html.*$/,'')+'remote.html?code='+hostCode,width:200,height:200})});peer.on('connection',c=>{connections.add(c);status();c.on('open',()=>c.send({type:'state',state:state()}));c.on('data',m=>m.type==='command'&&action(m.action));c.on('close',()=>{connections.delete(c);status()})});peer.on('error',e=>{if(e.type==='unavailable-id'){peer.destroy();setTimeout(host,100)}else toast('Erreur de connexion')});peer.on('disconnected',()=>peer.reconnect())}
qs('#connect').onclick=()=>qs('#modal').classList.add('open');qs('#closeModal').onclick=()=>qs('#modal').classList.remove('open');
let hide;qs('#stage').addEventListener('mousemove',()=>{qs('#stage').classList.add('show-controls');clearTimeout(hide);hide=setTimeout(()=>qs('#stage').classList.remove('show-controls'),2500)});host();ui();render();
